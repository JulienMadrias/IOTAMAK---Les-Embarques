...
class ControleurSMA(Controleur):
    ...
    def add_ant(self, ant):
        ...
        ant_ihm.id_image = self.draw_image(ant.get_dx(), ant.get_dy(
        ), ControleurSMA.color_to_file.get(ant_ihm.color))
        ...
        for ant in self.get_amas().get_agents():
            ...
            for ant_ihm in self.__ants:
                if ant.get_id() == ant_ihm.agent_id:
                    seen = True
                    self.move_image(ant_ihm.id_image,
                                    ant.get_dx(), ant.get_dy())
